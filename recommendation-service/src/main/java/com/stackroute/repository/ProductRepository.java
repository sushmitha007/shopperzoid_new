...
public interface ProductRepository extends Neo4jRepository<Products,Long> {
    ...
    @Query("Match (p1:Products),(p2:Products) where NOT id(p1)=id(p2) AND id(p1)={productId} AND p1.productBrandName=p2.productBrandName AND NOT p1.productSubCategory=p2.productSubCategory create (p1)-[r:RelatedProduct]->(p2)")
    void postProductSimilarRelation(@Param("productId") Long productId);

    @Query("Match (p1:Products),(p2:Products) where NOT id(p1)=id(p2) AND id(p1)={productId} AND p1.productBrandName=p2.productBrandName AND NOT p1.productSubCategory=p2.productSubCategory create (p2)-[r:RelatedProduct]->(p1)")
