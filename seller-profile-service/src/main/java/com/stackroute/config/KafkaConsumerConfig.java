...
public class KafkaConsumerConfig {
    ...
    @Bean
    public ConsumerFactory<String, ProductsDto> productsDtoConsumerFactory()
    {
        Map<String,Object> config=new HashMap<>();
        config.put(ConsumerConfig.BOOTSTRAP_SERVERS_CONFIG,"localhost:9092");
        config.put(ConsumerConfig.GROUP_ID_CONFIG,"product-id");
        config.put(ConsumerConfig.KEY_DESERIALIZER_CLASS_CONFIG, StringDeserializer.class);
        config.put(ConsumerConfig.VALUE_DESERIALIZER_CLASS_CONFIG, JsonDeserializer.class);
        return new DefaultKafkaConsumerFactory<String,ProductsDto>(config,new StringDeserializer(),new JsonDeserializer<>(ProductsDto.class));
    }
    @Bean
    public ConcurrentKafkaListenerContainerFactory<String,ProductsDto> kafkaListenerContainerFactory2()
    {
        ConcurrentKafkaListenerContainerFactory<String,ProductsDto> factory=new ConcurrentKafkaListenerContainerFactory();
        factory.setConsumerFactory(productsDtoConsumerFactory());
        return factory;
    }
}
