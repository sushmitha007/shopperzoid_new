...
public class AuthUserService implements UserDetailsService {
  ...
  //Loads any user with matching username or in our case, email Id.
	public UserDetails loadUserByUsername(String username) throws UsernameNotFoundException {
		AuthUser user = authUserRepository.findByEmailId(username);
			return JwtUserFactory.create(user);
	}

	//Saves a user in the database which comes from Kafka message bus after registration
	...
	//Kafka listeners to set User DTO instance to receive info about a newly registered user.
